# 🛡️ SentraNova AI Solutions – ISO 27001 Mini Compliance Program

**Document Reference:** SNOVA-ISO27001-001  
**Version:** 1.0 | **Status:** Completed  
**Classification:** Public Portfolio  
**Author:** Ariel Bethea / Compliance Officer  
**Effective Date:** December 30, 2025   

---

## 📌 Overview
This project demonstrates the development of a mini compliance program for SentraNova AI Solutions, a fictional AI company. The program aligns with ISO 27001:2022 Annex A controls and simulates audit readiness for a startup environment.

This project uses fictional data for demonstration purposes only.

---

## 🧠 Purpose
To design a baseline Information Security Management System (ISMS) for a small organization preparing for ISO 27001 certification, focusing on risk-based control implementation and asset-based scoping.

---

## 🔍 Key Highlights
- ✅ ISO 27001 Annex A control matrix with:
  - Control ID, Requirement, Current Status, Owner
  - Maturity Level, Target Date, Risk Context, Dependencies, Priority
- ✅ Asset Register with:
  - Classification, Criticality, Responsible Control
  - Backup, Encryption, Patch Status
- ✅ Phased compliance roadmap for audit readiness
- ✅ Risk-based prioritization and dependency mapping  

---

## 📁 Included Sections
- Scope & Compliance Goals
- Objectives & Methodology
- ISO 27001 Control Matrix
- Asset Register
- Compliance Roadmap  

---

## 📸 Visual Preview
- [*Figure 1: ISO 27001 Control Matrix*](./ISO27001_Controls_Screenshot.png)  

- [*Figure 2: Asset Register*](./ISO27001_AssetRegister_Screenshot.png)  

---

## 📂 Repository Contents
- 📊 [Compliance_Program_ISO27001_Controls_AssetRegister.xlsx](./Compliance_Program_ISO27001_Controls_AssetRegister.xlsx): Contains ISO 27001 Annex A control matrix with risk context, maturity levels, dependencies, and priority, plus an asset register mapping controls to critical assets.  
  
- 📄 [Compliance_Program_Summary_ISO27001.pdf](./Compliance_Program_Summary_ISO27001.pdf): One-page summary outlining scope, compliance goals, objectives, methodology, and phased roadmap for audit readiness.  
  
- 📝 `README.md` (this file)  

---

## 🧩 Skills Demonstrated
- ISO 27001 Control Mapping & Gap Analysis  
- Risk Context Development  
- Asset-Based Compliance Scoping  
- Governance Documentation  
- Roadmap Planning for Audit Readiness  

---

## 🎯 Ideal For Roles In
- IT GRC Analyst  
- Compliance Specialist  
- Risk & Security Governance Analyst  
- Information Security Auditor (ISO 27001)  

---

## 🏷️ Tags
`#ISO27001` `#GRC` `#Compliance` `#InformationSecurity`   
`#RiskManagement` `#AuditReadiness`

---

## 🤝 Connect
Want to discuss risk frameworks or GRC documentation? Let’s connect:  
🔗 [LinkedIn](https://www.linkedin.com/in/arielbethea)

---
